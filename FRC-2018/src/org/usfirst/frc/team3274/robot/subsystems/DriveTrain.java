...
public class DriveTrain extends Subsystem {
	...
	// Four wheels
	private TalonSRX _rightMotor = new TalonSRXGroup(RobotMap.REAR_RIGHT_MOTOR, RobotMap.FRONT_RIGHT_MOTOR, RobotMap.RIGHT_MOTOR);
	private TalonSRX _leftMotor = new TalonSRXGroup(RobotMap.FRONT_LEFT_MOTOR, RobotMap.REAR_LEFT_MOTOR, RobotMap.LEFT_MOTOR);
	
	private Encoder rightEncoder = new Encoder(RobotMap.RIGHT_ENCODER[0], RobotMap.RIGHT_ENCODER[1], true,
			EncodingType.k4X);
	private Encoder leftEncoder = new Encoder(RobotMap.LEFT_ENCODER[0], RobotMap.LEFT_ENCODER[1], true,
			EncodingType.k4X);
	...
	public DriveTrain() {
		...
		} else { // Convert to feet 4in diameter wheels with 256 tick simulated
					// encoders
			rightEncoder.setDistancePerPulse(
					(WHEEL_DIAMETER/* in */ * Math.PI) / (ENCODER_PULSES_PER_REVOLUTION * 12.0/* in/ft */));
	 ...
	 * 
	 * @param dividor
	 *            - 1 is not sniper mode, anything between 0 and 1 is the percentage
	 *            the motors are reduced by
	 */
	...
	/**
	 * For joystick axis. Checks if the given axis is OI.DEAD_ZONE distance from 0.
	 * 
	 * @param axis
	 *            - joystick axis
	...
	public void tankDrive(double leftAxis, double rightAxis) {
	    ...
	    _leftMotor.set(ControlMode.PercentOutput, correctedPow[0]);
	    _rightMotor.set(ControlMode.PercentOutput, correctedPow[1]);
	...
	public void carDrive(Joystick joy) {
		this.carDrive(joy.getRawAxis(RobotMap.XBOX_LEFT_Y_AXIS), joy.getRawAxis(RobotMap.XBOX_RIGHT_X_AXIS));
	}
	...
	/**
	 * Calculates a correction for the speed the motors should be moving at based on
	 * difference in encoder readings.
	 * 
	 * Makes sure that the robot is moving in the direction it should be moving in,
	 * AKA perfectly straight if it should be going straight.
	 * 
	 * @param leftPower
	...
	public double[] getSpeedCorrection(double leftPower, double rightPower) {
		...
		System.out.println(String.format("LeftEncoder Value = %f ", leftEncoder.getRate()));
		return corrected;
	}
